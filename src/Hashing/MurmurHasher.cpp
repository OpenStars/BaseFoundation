...
namespace openstars {
    namespace hashing {
        ...
        HashType MurmurHasher::hash(const std::string& aKey) const {
            return MurmurHashString(aKey);
        }

        HashType MurmurHasher::hash(const int32_t aKey) const {
            return MurmurHash64A((void*) &aKey, sizeof (aKey));
        }

        HashType MurmurHasher::hash(const int64_t aKey) const {
            return MurmurHash64A((void*) &aKey, sizeof (aKey));
        }

        HashType MurmurHasher::hash(const void* aKey, int keySize) const {
            return MurmurHasher::MurmurHash64A(aKey, keySize, 0xdeadbeef * 16);
        }

        uint64_t MurmurHasher::MurmurHashString(const string& key) {
            return MurmurHasher::MurmurHash64A(key.c_str(), key.length(), 0xdeadbeef * 16);
        }
        ...
        uint64_t MurmurHasher::MurmurHash64A(const void * key, int len) {
            static unsigned int seed = 0xdeadbeef * 16;
        ...
        uint64_t MurmurHasher::MurmurHash64A(const void * key, int len, unsigned int seed) {
...
#define _rotl(A,n) (((A) << n) | ((A) >> (32-n)))

#define _rotr(A,n) (((A) >> n) | ((A) << (32-n)))



#define _rotl64(A,n) (((A) << n) | ((A) >> (64-n)))

#define _rotr64(A,n) (((A) >> n) | ((A) << (64-n)))
        ...
        //  . It's also not all that fast due to register spillage.
        ...
        void MurmurHash3_x64_128(const void * key, const int len, const uint32_t seed, void * out) {
            ...
            uint64_t h1 = 0x9368e53c2f6af274LLU ^ seed;
            uint64_t h2 = 0x586dcd208f7cd3fdLLU ^ seed;
