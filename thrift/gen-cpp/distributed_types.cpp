...
const std::map<int, const char*> _TProtocolType_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(9, _kTProtocolTypeValues, _kTProtocolTypeNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));
...
std::string to_string(const TProtocolType::type& val) {
  std::map<int, const char*>::const_iterator it = _TProtocolType_VALUES_TO_NAMES.find(val);
  if (it != _TProtocolType_VALUES_TO_NAMES.end()) {
...
const std::map<int, const char*> _TBackendStatus_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(4, _kTBackendStatusValues, _kTBackendStatusNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));
...
std::string to_string(const TBackendStatus::type& val) {
  std::map<int, const char*>::const_iterator it = _TBackendStatus_VALUES_TO_NAMES.find(val);
  if (it != _TBackendStatus_VALUES_TO_NAMES.end()) {
    return std::string(it->second);
  } else {
...
TDeadBackend::~TDeadBackend() noexcept {
}
...
const char* TDeadBackend::what() const noexcept {
  try {
    std::stringstream ss;
...
TDataMovingStatus::~TDataMovingStatus() noexcept {
}
...
TDataMovingStatus::TDataMovingStatus(const TDataMovingStatus& other2) noexcept {
...
TDataMovingStatus& TDataMovingStatus::operator=(const TDataMovingStatus& other3) noexcept {
    ...
    switch (fid)
    {
      ...
      case 5:
        if (ftype == ::apache::thrift::protocol::T_I32) {
          ...
          this->status = static_cast<TBackendStatus::type>(ecast4);
