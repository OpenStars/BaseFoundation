...
public:
    ...
    static int32_t Deserialize(TType&value, const std::string &strBuf) {
        ...
        if (snappy::Uncompress(strBuf.data(), strBuf.length(), &tmp)) {
            boost::shared_ptr<TMemoryBuffer> memBuf(new TMemoryBuffer((uint8_t*) tmp.data(), tmp.size(), TMemoryBuffer::OBSERVE));
...
public:
    ...
    static int Serialize(boost::shared_ptr<TMemoryBuffer> &memBuf, const TType& value) {
        try {
            if (memBuf == 0)
                memBuf = boost::shared_ptr<TMemoryBuffer > (new TMemoryBuffer());
    ...
    static int Deserialize(TType& value, const boost::shared_ptr<TMemoryBuffer>& memBuf) {
        if (!memBuf)
            return -1;
    ...
    static int32_t SerializeT(std::string& strBuf, const TType& value) {
        ...
        boost::shared_ptr<TMemoryBuffer > buf = boost::shared_ptr<TMemoryBuffer > (new TMemoryBuffer());
    ...
    static int32_t DeserializeT(TType& value, const std::string& strBuf) {
        ...
        if (snappy::GetUncompressedLength(strBuf.data(), strBuf.length(), &uncompressSize)) {
            if (uncompressSize < 1L << 32) {
                ...
                boost::shared_ptr<TMemoryBuffer > buf =
                        boost::shared_ptr<TMemoryBuffer > (new TMemoryBuffer(uncompressSize));
		...
		boost::shared_ptr<TMemoryBuffer> buf(new TMemoryBuffer((uint8_t*)strBuf.data(), (uint32_t)strBuf.length()));
        return Deserialize(value, buf);
    }
