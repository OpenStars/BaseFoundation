...
#define CachePersistent_Tmpl     template <class _TKey, class _TValue, class _ObjectStorage >
#define CachePersistent_Clss CachePersistent<_TKey, _TValue, _ObjectStorage>
    
template <class _TKey, class _TValue, class _ObjectStorage >
class CachePersistent
    ...
    public:
        ...
        typedef CachePersistent<_TKey, _TValue, _ObjectStorage> ThisClass;
	...
	void setNumWarmingThread(int numThread);
        ...
        bool popDirtyKeyValue(TKey& key, TValue& value)
        {
            ...
            {
                return _cache->popHeadDirty(key, value);
            }
    ...
    protected:
        ...
        bool _enableWarmingUp;
        bool _enableSavingThread;
    ...
    private:
        ...
        public:
            visit_manipulator( typename CachePersistent_Clss::data_visitor* aVisistor, bool needCopy = false):_visitor(aVisistor), _visited(false), _changed(false), _needCopy(needCopy){}
        public:
...
void CachePersistent_Clss::visit(const _TKey& key, data_visitor* aVisistor){
    ...
    if (manipulator._changed ){
        if (this->_asyncFlush && _enableAsyncFlush)
            ...
            if (this->_storage && manipulator._copiedValue)
            {
...
{
    if (_smartSaving && !this->_enableSavingThread )
    {
